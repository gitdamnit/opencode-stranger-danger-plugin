...
export const strangerDanger = async (_ctx) => {
        ...
        tool: {
            ...
            sd_allowlist_show: tool({
                ...
                async execute() {
                    ...
                    return allowlist.entries.map((e, i) => `${i + 1}. ${e.description || "(no description)"} | fingerprint: ${e.fingerprint || "—"} | ruleId: ${e.ruleId || "—"} | path: ${e.path || "—"} | regex: ${e.regex || "—"}`).join("\n");
                },
            }),
            sd_scan: tool({
                ...
                async execute(args) {
                    ...
                    const { findings } = scanAndRedact(args.content, src);
                    if (findings.length === 0)
                        return "No secrets detected.";
                    return findings.map((f) => `${f.ruleId}: ${f.description}\n  Redacted: ${f.redacted}\n  Offset: ${f.offset}`).join("\n\n");
                },
            }),
        },
    };
