 ...
 */
function minipath(p: string): string {
    return p.replace(/\\/g, "/").replace(/^\.\//, "").toLowerCase();
}

const DEFAULT_ALLOWLIST_PATH = ".strangerdanger-allowlist.toml";

function parseSimpleToml(content: string): AllowlistEntry[] {
    ...
    for (const rawLine of content.split("\n")) {
        ...
        current[key as keyof AllowlistEntry] = value;
    }
...
function serializeTomlEntry(entry: AllowlistEntry): string {
    const lines = ["[[entries]]"];
    if (entry.description) lines.push(`description = "${entry.description}"`);
    if (entry.fingerprint) lines.push(`fingerprint = "${entry.fingerprint}"`);
    if (entry.ruleId) lines.push(`ruleId = "${entry.ruleId}"`);
    if (entry.path) lines.push(`path = "${entry.path}"`);
    if (entry.regex) lines.push(`regex = "${entry.regex}"`);
    return lines.join("\n");
}
...
export function saveAllowlistEntry(entry: AllowlistEntry, path?: string): void {
    const target = path || DEFAULT_ALLOWLIST_PATH;
    const block = serializeTomlEntry(entry) + "\n\n";
    appendFileSync(target, block, "utf8");
}

export function filterByAllowlist(findings: Finding[], allowlist: Allowlist): Finding[] {
    if (allowlist.entries.length === 0) return findings;

    return findings.filter((finding) => {
        for (const entry of allowlist.entries) {
            if (entry.fingerprint) {
                const fp = finding.redacted.match(/:([a-f0-9]{8})\]/);
                if (fp && fp[1] === entry.fingerprint) return false;
            }
            if (entry.ruleId && finding.ruleId === entry.ruleId) return false;
            if (entry.path) {
                 if (finding.source === "file" && finding.filePath &&
                     minipath(finding.filePath).includes(minipath(entry.path))) return false;
             }
            if (entry.regex) {
                try {
                    const re = new RegExp(entry.regex);
                    if (re.test(finding.match)) return false;
                } catch {
